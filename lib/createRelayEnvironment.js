...
export default function initEnvironment({ records = {} } = {}) {
  ...
  const createEnvironment = () => {
    ...
    // A workaround because react-relay-offline's Store does not
    // persist the records passed to it during instantiation:
    // https://github.com/morrys/react-relay-offline/issues/23
    // const store = new Store(new RecordSource(records))
    const store = new Store()
    const cache = store.getSource()
    Object.keys(records).forEach(key => {
      cache.set(key, records[key])
    })
