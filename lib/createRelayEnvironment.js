...
export default function initEnvironment({ records = {} } = {}) {
  ...
  const createEnvironment = () => {
    ...
    // Note: no records when using react-relay-offline, but
    // records exist when using relay-runtime.
    // FIXME: this is probably the primary cause of failing
    // SSR when using react-relay-offline.
    // https://github.com/morrys/react-relay-offline/issues/23
    console.log('store records', store.getSource().toJSON())
